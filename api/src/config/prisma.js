 ...
 */
function applyTenantFilter(model, args) {
 ...
 */
function applyTenantData(model, args) {
    if (!MULTI_TENANT_MODELS.includes(model))
        return args;
    const tenantId = TenantContext.getTenantId();
    if (!tenantId)
    ...
    get(target, prop) {
        if (prop === 'user') {
            return {
                ...
                upsert: async (args) => {
                    try {
                        return await target.user.upsert(args);
                    }
                    catch (e) {
                ...
                update: async (args) => {
                    ...
                    if (tenantId) {
                        ...
                        if (existing && existing.tenant_id && existing.tenant_id !== tenantId)
                            throw new Error("Forbidden cross-tenant update");
                    }
                    try {
                        return await target.user.update(args);
                    }
                    catch (e) {
        ...
        if (prop === 'session') {
            return {
                create: async (args) => {
                    try {
                        return await target.session.create(args);
                    }
                    catch (e) {
                ...
                findUnique: async (args) => {
                    try {
                        return await target.session.findUnique(args);
                    }
                    catch (e) {
                ...
                update: async (args) => {
                    try {
                        return await target.session.update(args);
                    }
                    catch (e) {
                        const session = inMemoryDb.sessions.find(s => s.id === args.where.id);
                        if (session)
        ...
        if (prop === 'tenant') {
            return {
                findFirst: async () => {
                    try {
                        return await target.tenant.findFirst();
                    }
                    catch (e) {
                ...
                create: async (args) => {
                    try {
                        return await target.tenant.create(args);
                    }
                    catch (e) {
                        return { id: 1, name: "Default Tenant" };
                    }
        ...
        if (prop === 'role') {
            return {
                findUnique: async (args) => { try {
                    return await target.role.findUnique(args);
                }
                catch (e) {
                    return { id: 1, code: args?.where?.code || "CHEF_PROJET" };
                } },
                create: async (args) => { try {
                    return await target.role.create(args);
                }
                catch (e) {
                    return { id: 1, code: args?.data?.code || "CHEF_PROJET" };
                } },
                upsert: async (args) => { try {
                    return await target.role.upsert(args);
                }
                catch (e) {
                    return { id: 1, code: args?.create?.code || "CHEF_PROJET" };
                } }
            };
        }
        if (prop === 'project') {
            return {
                findMany: async (args) => { try {
                    return await target.project.findMany(args);
                }
                catch (e) {
                    return [];
                } },
                upsert: async (args) => { try {
                    return await target.project.upsert(args);
                }
                catch (e) {
        ...
        if (prop === 'userRole') {
            return {
                create: async (args) => { try {
                    return await target.userRole.create(args);
                }
                catch (e) {
                    return args.data;
                } },
                findMany: async (args) => {
                    try {
                        return await target.userRole.findMany(args);
                    }
                    catch (e) {
        ...
        if (original && typeof original === 'object' && !['tenant', 'session'].includes(prop)) {
            return new Proxy(original, {
                get(subTarget, subProp) {
                    const subOriginal = subTarget[subProp];
                    if (typeof subOriginal === 'function') {
                        return async (...args) => {
                            let finalArgs = args[0] || {};
                            const tenantId = TenantContext.getTenantId();
                            if (['findMany', 'findFirst', 'count', 'aggregate', 'groupBy'].includes(subProp)) {
                                finalArgs = applyTenantFilter(prop, finalArgs);
                            }
                            else if (['create', 'createMany'].includes(subProp)) {
                                finalArgs = applyTenantData(prop, finalArgs);
                            }
                            else if (['update', 'delete', 'updateMany', 'deleteMany', 'upsert'].includes(subProp)) {
                                if (subProp.toString().endsWith('Many')) {
                                    finalArgs = applyTenantFilter(prop, finalArgs);
                                }
                                else if (tenantId) {
                                    // Verification before single update/delete/upsert
                                    const existing = await subTarget.findUnique({ where: finalArgs.where }).catch(() => null);
                                    if (existing && existing.tenant_id && existing.tenant_id !== tenantId) {
                            ...
                            try {
                                return await subOriginal.apply(subTarget, [finalArgs]);
                            }
                            catch (e) {
        ...
        if (typeof original === 'function') {
            return async (...args) => {
                try {
                    ...
                    console.error("Mock fallback for", prop);
                    return [];
                }
            };
        }
