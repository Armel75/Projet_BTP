...
export class WeeklyReportService {
    static async generateWeeklyReport(data) {
        ...
        const totalLaborHours = dailyLogs.reduce((sum, log) => sum + log.labor_entries.reduce((lSum, l) => lSum + l.hours, 0), 0);
        ...
        if (tasks.length > 0) {
            const items = tasks.slice(0, 10).map((task) => ({
                report_id: report.id,
                task_id: task.id,
    ...
    static async getWeeklyReports(project_id) {
        const tenantId = TenantContext.getTenantId();
        if (!tenantId)
            throw new Error("Tenant session required");
    ...
    static async updateWeeklyReport(id, data) {
        return await prisma.$transaction(async (tx) => {
            if (data.items) {
                ...
                await tx.weeklyReportItem.createMany({
                    data: data.items.map((it) => ({ ...it, report_id: id }))
                });
            }
